...
---Cappitalization
select distinct  '1' as 'NoLow',FixedAss.ObjType,FixedAss.CompanyName,FixedAss.AsClassCode,FixedAss.AsClassName,FixedAss.AsGroup,FixedAss.ItemCode,FixedAss.ItemName,FixedAss.DeperRate,FixedAss.CapDate,ISNULL(FixedAss.PriceCost,0) 'PriceCost',ISNULL(FixedAss.Deperciation,0) 'Deperciation' 
...
Union all
select distinct  '2' as 'NoLow'
,FixedAss.ObjType
,null as CompanyName
,FixedAss.AsClassCode 
,FixedAss.AsClassName
,null as AsGroup
,FixedAss.ItemCode
,'   Credit Memo' as ItemName
,null as DeperRate
,FixedAss.ACDDATE as 'CapDate'
,ISNULL(-FixedAss.PriceCost,0) 'PriceCost'
,0 as 'Deperciation' 
,0 as 'PriceNet'
,@dateFrom 'DateFrom',@dateTo 'DateTo'
from (select distinct [@SLDT_SET_BRANCH].U_SLD_VComName as 'CompanyName'
...
, CONCAT( (100/(ITM7.UsefulLife/12)),' ', '%') 'DeperRate' --, (ITM7.UsefulLife) 'Usefull Life M'
,FXCN.ACDDATE
--,case when FXCAP.CNSum is null and FXCAP.CNSum = 0 then TopITM8.APC else FXCAP.CapSum  end 'PriceCost'  --,TopITM8.APC,FXCAP.CapSum
,FXCN.CNSum 'PriceCost'
,Deperciation2.Deperciation
,FXCN.ObjType
from OITM T0 
LEFT JOIN OACS ON T0.AssetClass = OACS.Code
...
---Pull Deperciation
	left join (select De2.ItemCode,sum(De2.OrdDprPost) 'Deperciation' from 
				(select itemcode, OrdDprPost from odpv where odpv.PeriodCat = year(@dateTo) ) De2 group by itemCode)
					Deperciation2 on Deperciation2.ItemCode = T0.itemcode
---Capitalization Credit Memo
	left join ( select ACD.ItemCode,sum(ACD.LineTotal) 'CNSum' ,ACD.ObjType,ACD.ACDDate from (select convert(varchar,OACD.PostDate, 23) 'ACDDate', ACD1.itemcode, ACD1.Linetotal ,OACD.Objtype
				from OACD inner join ACD1 on OACD.DocEntry = ACD1.DocEntry where OACD.DocStatus = 'P'
					) ACD where (ACD.ACDDate >= @dateFrom and ACD.ACDDate <= @dateTo)   group by ACD.ItemCode , ACD.Objtype, ACD.ACDDate) FXCN on T0.ITEMCODE = FXCN.itemCode	
---Pull APC 
--	left join ( select distinct itemcode,APC  from itm8 where APC is not null and APC <> 0 ) TopITM8 on T0.ITEMCODE = TopITM8.itemCode 
...
,null as AsGroup
,FixedAss.ItemCode
,'   Revaluation' as ItemName
,null as DeperRate
,FixedAss.RevaluDate  'CapDate'
,ISNULL(-FixedAss.PriceCost,0) 'PriceCost'
,0 as 'Deperciation' 
,0 as 'PriceNet'
,@dateFrom 'DateFrom',@dateTo 'DateTo'
...
---Capitalization Credit Memo
	left join (select Revalu.RevaluDate,Revalu.ItemCode,sum(Revalu.NBV)-sum(Revalu.New_NBV) 'SumRevalu' 
			from (select convert(varchar,OFAR.PostDate, 23) 'RevaluDate' ,FAR1.itemcode, FAR1.NBV, FAR1.New_NBV --,sum(NBV) 'Sum1',sum(New_NBV) 'Sum2'
				from OFAR INNER JOIN FAR1 on FAR1.DocEntry = OFAR.DocEntry
					) Revalu where (Revalu.RevaluDate >= @dateFrom and Revalu.RevaluDate <= @dateTo) group by Revalu.RevaluDate,Revalu.ItemCode) FXRE on T0.ITEMCODE = FXRe.itemCode
---Pull APC 
--	left join ( select distinct itemcode,APC  from itm8 where APC is not null and APC <> 0 ) TopITM8 on T0.ITEMCODE = TopITM8.itemCode 

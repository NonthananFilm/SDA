...
LEFT JOIN OCPR ON ORPD.CntctCode = OCPR.CntctCode 
LEFT JOIN NNM1 ON ORPD.Series = NNM1.Series 
LEFT JOIN OCTG ON ORPD.GroupNum = OCTG.GroupNum
LEFT JOIN OADM ON ORPD.DocCur = OADM.MainCurncy
LEFT JOIN [dbo].[@SLDT_SET_BRANCH] BRANCH ON ORPD.U_SLD_LVatBranch = BRANCH.Code
---------------------------------------------Serial-----------------------------------------------------
LEFT JOIN (SELECT *,
...
WHERE PV.BaseType = 21) As rRow ) As Serial ON ORPD.DocEntry = Serial.BaseEntry AND RPD1.LineNum = Serial.BaseLinNum AND RPD1.ItemCode = Serial.ItemCode
-------------------------------------------------------BATCH---------------------------------------------------------------------------------------
LEFT JOIN (SELECT *,
...
WHERE PVB.BaseType = 21) As BATCH ) As Batch_N ON ORPD.DocEntry = Batch_N.BaseEntry AND RPD1.LineNum = Batch_N.BaseLinNum AND RPD1.ItemCode = Batch_N.ItemCode

ORDER BY (RPD1.VisOrder+1)

...
LEFT JOIN OADM ON OPDN.DocCur = OADM.MainCurncy
LEFT JOIN [dbo].[@SLDT_SET_BRANCH] BRANCH ON OPDN.U_SLD_LVatBranch = BRANCH.Code
---------------------------------------------Serial-----------------------------------------------------
LEFT JOIN (SELECT *,
...
WHERE PV.BaseType = 20) As rRow ) As Serial ON OPDN.DocEntry = Serial.BaseEntry AND PDN1.LineNum = Serial.BaseLinNum AND PDN1.ItemCode = Serial.ItemCode
-------------------------------------------------------BATCH---------------------------------------------------------------------------------------
LEFT JOIN (SELECT *,
...
WHERE PVB.BaseType = 20) As BATCH ) As Batch_N ON OPDN.DocEntry = Batch_N.BaseEntry AND PDN1.LineNum = Batch_N.BaseLinNum AND PDN1.ItemCode = Batch_N.ItemCode

ORDER BY (PDN1.VisOrder+1)

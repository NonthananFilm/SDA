...
PCH1.Dscription, 
PCH1.Quantity,
CAST(CRD1.Building AS NVARCHAR(200)) AS 'BPBuilding',
PCH1.PriceBefDi, 
...
LEFT JOIN CRD1 ON (OPCH.PaytoCode = CRD1.[Address] AND OPCH.CardCode = CRD1.CardCode AND CRD1.AdresType ='B' ) 
...
LEFT JOIN OADM ON OPCH.DocCur = OADM.MainCurncy
...
WHERE PV.BaseType = 18) As rRow ) As Serial ON OPCH.DocEntry = Serial.BaseEntry AND PCH1.LineNum = Serial.BaseLinNum AND PCH1.ItemCode = Serial.ItemCode
-------------------------------------------------------BATCH---------------------------------------------------------------------------------------
LEFT JOIN (SELECT *,
...
WHERE PVB.BaseType = 18) As BATCH ) As Batch_N ON OPCH.DocEntry = Batch_N.BaseEntry AND PCH1.LineNum = Batch_N.BaseLinNum AND PCH1.ItemCode = Batch_N.ItemCode

ORDER BY (PCH1.VisOrder+1)

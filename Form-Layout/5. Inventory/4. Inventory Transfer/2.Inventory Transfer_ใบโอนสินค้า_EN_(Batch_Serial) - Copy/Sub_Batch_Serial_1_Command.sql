...
WHERE PV.BaseType = 67) As rRow ) As Serial ON OWTR.DocEntry = Serial.BaseEntry AND WTR1.LineNum = Serial.BaseLinNum AND WTR1.ItemCode = Serial.ItemCode
-------------------------------------------------------BATCH---------------------------------------------------------------------------------------
LEFT JOIN (SELECT *,
...
WHERE PVB.BaseType = 67) As BATCH ) As Batch_N ON OWTR.DocEntry = Batch_N.BaseEntry AND WTR1.LineNum = Batch_N.BaseLinNum AND WTR1.ItemCode = Batch_N.ItemCode

ORDER BY (WTR1.VisOrder+1)

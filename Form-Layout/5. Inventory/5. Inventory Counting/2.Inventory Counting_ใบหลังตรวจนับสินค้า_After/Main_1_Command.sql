...
--,T1.[Difference]
,T1.Counted 'StatusCount'
,case when T1.Counted = 'Y' then T1.InWhsQty else '0' end as 'InWhs'
,case when T1.Counted = 'Y' then T1.CountQty else '0' end as 'CountedQ'
,case when T1.Counted = 'Y' then T1.[Difference] else '0' end as 'Diff'
...
LEFT JOIN obtn t6 on  T5.[MdAbsEntry] = T6.[AbsEntry] and T5.[WhsCode] = T1.[WhsCode]
